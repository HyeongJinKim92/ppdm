...
paillier_pubkey_t* pub = 0;
paillier_prvkey_t* prv = 0;
...
void	paillier_setkey(paillier_pubkey_t* pubkey, paillier_prvkey_t* prvkey)
{
	pub = pubkey;
	prv = prvkey;
}
...
void paillier_subtract( paillier_pubkey_t* pub,	paillier_ciphertext_t* res, paillier_ciphertext_t* ct0,	paillier_ciphertext_t* ct1 )
{
	paillier_ciphertext_t* minus = (paillier_ciphertext_t*)malloc(sizeof(paillier_ciphertext_t));
	mpz_init(minus->c);
	//gmp_printf("minus : %Zd\n", minus);

	paillier_plaintext_t* a = paillier_plaintext_from_ui(0);
	mpz_sub_ui(a->m, a->m, 1); // 플레인 -1
	//gmp_printf("a : %Zd\n", a);

	mpz_powm(minus->c, ct1->c, a->m, pub->n_squared);
	
	mpz_mul(res->c, ct0->c, minus->c);
	mpz_mod(res->c, res->c, pub->n_squared);
	//a=paillier_dec(0, pub, prv, res);
	//gmp_printf("a : %Zd\n", a);

	paillier_freeciphertext(minus);
}

void
paillier_divide( paillier_pubkey_t* pub,
						 	paillier_ciphertext_t* res,
						 	paillier_ciphertext_t* ct0,
						 	paillier_plaintext_t* pt0 )
{
	//gmp_printf("tmp : %Zd\n", pt0);
	paillier_plaintext_t* p_minus = paillier_plaintext_from_ui(0); 
	mpz_sub_ui(p_minus->m, p_minus->m, 1); // 플레인 -1
	//gmp_printf("a : %Zd\n", p_minus);

	mpz_powm(pt0->m, pt0->m, p_minus->m, pub->n_squared);
	mpz_powm(res->c, ct0->c, pt0->m, pub->n_squared);
}
...
paillier_create_enc_one()
{
	...
	ct = (paillier_ciphertext_t*) malloc(sizeof(paillier_ciphertext_t));
	a=paillier_plaintext_from_ui(1);
    ct=paillier_enc(0, pub, a, paillier_get_rand_devurandom);
	free(a);
	return ct;
}
paillier_ciphertext_t* 
paillier_create_enc_zero()
{
	...
	/* note that this is just the number 0 */

	ct = (paillier_ciphertext_t*) malloc(sizeof(paillier_ciphertext_t));
	a=paillier_plaintext_from_ui(0);
    ct=paillier_enc(0, pub, a, paillier_get_rand_devurandom);
	free(a);
	return ct;
}
paillier_ciphertext_t* 
paillier_create_enc(int num)
{
	paillier_plaintext_t* a;
	paillier_ciphertext_t* ct;
	...
	ct = (paillier_ciphertext_t*) malloc(sizeof(paillier_ciphertext_t));
	a=paillier_plaintext_from_ui(num);
    ct=paillier_enc(0, pub, a, paillier_get_rand_devurandom);
	free(a);
	return ct;
}
/*
void
paillier_print(paillier_ciphertext_t*  msg){
	paillier_plaintext_t* m;		
	m = paillier_dec(0, pub, prv, msg);
	gmp_printf("%Zd\n", m);
...
void
paillier_print(char * str, paillier_ciphertext_t*  msg){
	paillier_plaintext_t* m = paillier_dec(0, pub, prv, msg);
...
void
plain_print(char * str, paillier_plaintext_t*  msg){
	gmp_printf("%s%Zd\n", str, msg);
}

void
plain_mul( paillier_pubkey_t* pub,
						 	paillier_plaintext_t* res,
						 	paillier_plaintext_t* pt0,
						 	paillier_plaintext_t* pt1 )
{
	mpz_mul(res->m, pt0->m, pt1->m);
	mpz_mod(res->m, res->m, pub->n_squared);
}


void
paillier_print_bit(paillier_ciphertext_t*  msg){
	paillier_plaintext_t* m = paillier_dec(0, pub, prv, msg);
...
boundary getLLBound() {return rectangle.LL;}


boundary getURBound() {return rectangle.UR;}

void setLLBound(int val, int dimension){
	rectangle.LL.bound[dimension] = val;
}

void setURBound(int val, int dimension){
	rectangle.UR.bound[dimension] = val;
}

int getLLBound_dim(int dimension) {return rectangle.LL.bound[dimension];}
int getURBound_dim(int dimension) {return rectangle.UR.bound[dimension];}
*/